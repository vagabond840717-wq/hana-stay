...
```
MM/DD · 쉬운 이름              시작하고 그날 끝낸 것
MM/DD~DD · 쉬운 이름           며칠에 걸친 것 (시작일~마지막 손댄 날)
```
- **날짜가 먼저.** 사용자가 날짜로 기억하고 찾는다
- **다시 손대면 뒤 날짜를 그날로 갱신한다.** `09/22 · …` → 09/24 에 또 만졌으면 `09/22~24 · …`
- 달을 넘기면 `09/28~10/02 · …`
- **코딩 용어 금지.** 함수명·파일명 말고 **사용자가 겪은 증상**으로 적는다
```
❌  mergeArchiveIntoRooms 포함관계 필터 수정
✅  09/23 · 수첩이 진짜 예약을 지우던 문제
```
