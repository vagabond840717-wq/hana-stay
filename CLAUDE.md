...
**⚠ 이름 형식 — 세션 제목·NEXT.md·세이브 포인트 전부 똑같이 쓴다.**
```
09/22 유령 체크아웃          하루에 끝낸 것
09/22~24 유령 체크아웃       며칠 걸린 것 (뒤 날짜 = 마지막으로 손댄 날)
```
- **날짜 먼저, 한 칸 띄고, 짧은 이름.** 가운뎃점(·) 같은 기호 넣지 말 것
- **이름은 짧게.** 길면 사이드바에서 잘려서 못 알아본다
- **다시 손대면 뒤 날짜를 그날로 갱신.** `09/22 …` → 09/24 에 또 만졌으면 `09/22~24 …`
- 달을 넘기면 `09/28~10/02 …`
- **코딩 용어 금지.** 함수명·파일명 말고 **사용자가 겪은 증상**으로
```
❌  mergeArchiveIntoRooms 포함관계 필터 수정
❌  09/23 · 수첩이 진짜 예약을 지우던 문제 (길고 기호가 붙었다)
✅  09/23 수첩이 진짜 예약 지움
```

**⚠ 덮어썼으면 사용자에게 한 줄로 알린다** — 사용자가 세이브 포인트를 모르면 이 장치는 무용지물이다.
```
💾 세이브 포인트  09/24 트립 블락 표시
