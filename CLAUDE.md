...
수정 후:
- [ ] **예약앱을 고쳤다면 — 캐시를 비워야 반영된다.** 예약앱은 PWA라 서비스워커가 옛 index.html 을
      붙들고 있다. `?v=<난수>` 를 붙여 받으면 **새 코드가 보이는 게 당연하므로**, 그것만으로
      "배포 반영됨"이라고 말하면 사용자 화면과 다른 소리를 하게 된다 (2026-09-21 실제로 어긋났다).
      사용자에게 캐시 비우기를 먼저 안내할 것. 청소앱(Cloudflare Pages)은 PWA가 아니라 무관
