...
- **앞잘림 잔재 제거** (2026-09-23 좁힘 — known-issues #34): **채널이 `bk`·`tr` 이고, 끝날짜가 같고, 시작만 늦어진** 항목만 제거한다. 트립·부킹의 "오늘~체크아웃" 일일 스냅샷 적립을 막는다.
  - ⛔ 옛 규칙(**"더 넓은 것 안에 완전히 포함되면 제거"**, 2026.07.08 #17)으로 되돌리지 말 것 — **진짜 예약을 지운다.** 601호 부킹 7개월짜리 '안 파는 기간' 하나가 그 안의 진짜 예약 9건을 삼켰고, 손님이 예약을 줄이면 진짜가 짧은 쪽이 되어 매번 진짜가 졌다 (실측 12건 복구).
  - `ab` 는 앞을 자르지 않으므로 **아예 적용하지 않는다.**
  - 워커·예약앱 양쪽에 동일 규칙 (`UNTRIM_PLATFORMS` / `TRIM_PLATFORMS`) — 바꾸면 두 곳 동시 수정
...
## 피드 변화 기록 (feed_changes KV) — 2026-09-22

### 저장 위치
`HANA_KV: feed_changes` (⚠ `extra_` 접두사 없음). **워커만 쓴다** — 두 앱은 읽지 않는다.

### 구조
```js
[{ ts: 1790053912143,        // 관측 시각 (ms)
   room: "603 jnj",
   ch:   "ab",               // 'ab'|'bk'|'tr'|'lv'
   kind: "in" | "out",       // 피드에 나타남 / 사라짐
   cin:  "20260915",
   cout: "20260920",
   sum:  "Reserved" }]
```
최신이 배열 앞. 상한 **3000줄 / 120일**.

### 왜 필요한가
예약에 **고유 이름표가 없다.** 신원이 '날짜'인데 정작 바뀌는 게 날짜라, 하루만 달라져도
남남이 된다. #29·#31·#32·#34 가 전부 여기서 나온다. 해법은 **'언제 무엇이 바뀌었나'** 인데
그 기록이 어디에도 없었다 — `syncAllRooms` 가 매 5분 계산하던 `cancelled` 는 쓰는 데가 없었다.

알림 로그(`events`)로는 대체 불가: 50개 상한(≈11일치), `not available` 필터 때문에
부킹닷컴이 통째로 빠지고, **사라짐은 아예 안 적는다.**
...
- 피드 수신 실패 채널은 이전 값을 그대로 물려받으므로 **연결 오류가 '사라짐'으로 안 적힌다**
- 호실을 지웠을 때 그 방 예약이 통째로 `out` 으로 쏟아지지 않게, **새 목록에 있는 호실만** 본다
- ⚠ 동기화가 짧은 간격으로 두 번 돌면 같은 변화가 두 줄 적힐 수 있다 (KV 읽기 시차). 기록만의 문제

### 조회
`GET /changes?room=<호실>&ch=<채널>&limit=<개수>`

---

## 예약 메모 (memo-indicator) — 2026-09-25

### 서랍 이름 — 두 앱이 같은 이름을 쓴다
```
extra_<호실>|<입실일 YYYYMMDD>        ← 에어비앤비
extra_<호실>|<입실일>_bk | _tr | _lv  ← 부킹 · 트립 · 리브
extra_<호실>|<시작일>_bl              ← 수동 블락 (새 꼬리표)
{ passwords:[...], memos:[{text, date, ch, cin, cout}] }
```
- 예전엔 예약앱이 꼬리표 없이, 청소앱은 꼬리표를 붙여 써서 **부킹·트립·리브는 두 앱이 다른 서랍을 봤다** → 이번에 통일
- 메모 한 줄의 `ch·cin·cout` = 머리표 (`🏨 10/20~10/28`). **메모를 쓴 순간의 예약으로 고정**된다

### 메모 목록 — `extra_memo_index` (공유 배열)
```js
[{ key:"601 jnj|20261020", room:"601 jnj", ch:"ab", cin:"20261020", cout:"20261028", uid:"…@airbnb.com" }]
```
- 메모가 1개 이상 있는 서랍만 한 줄. 예약앱 달력 📝 는 **이 목록 하나만** 읽는다
- 앱에서 쓸 때는 반드시 `saveListMerged` (#22·#26). 못 읽으면 쓰지 않는다
- `uid` 는 **에어비앤비만** (워커 `parseIcal` 이 실어 보냄). 부킹은 UID 가 매일 바뀐다는 보고가 있어 싣지 않는다

### 워커가 하는 일 (`upkeepMemos`, 5분 동기화 끝)
- 에어비앤비 **날짜 변경 따라가기**: 같은 `uid` 가 다른 입실일로 보이면 메모를 새 서랍으로 옮긴다 (비밀번호는 안 옮김)
- **30일 정리** (하루 1회, `memo_cleanup_day`): `퇴실 + 30일 < 오늘` → 메모만 비우고 비밀번호는 남긴다. 퇴실일 모르면 `입실 + 90일`
- ⛔ 메모를 자동으로 지우는 길은 **30일 정리 하나뿐**. 예약이 피드에서 사라지면 📝 만 숨는다 (#27)
- 백업: 이관 때 바뀐 서랍 원본 `memo_backup_<시각>`. 30일 정리는 백업 없음
