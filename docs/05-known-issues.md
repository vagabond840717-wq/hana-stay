...
- 원본 피드가 바뀌면 예약앱은 먼저 **자동 승계 시도**(#18) → 승계 불가 시 자동 무효 + 안내(`notifyStaleSplits`). 데이터 로드 실패 시엔 무효 판정 보류(오삭제 방지).
...
### 18. 분할 자동 승계(split-inherit)는 예약앱 전용 로직 (2026.07.15)
직전 피드 스냅샷(`tr_feed_prev`) 비교로 통짜 합쳐짐을 감지해 잠정 분할(⚠ 배지)을 만드는 로직은 **예약앱에만** 있다.  
- 청소앱은 잠정 분할의 결과 조각만 반영 (KV의 `tr_splits`를 읽으므로 자동) — 단 피드 변경 직후 예약앱이 한 번 열리기 전까지는 청소앱에 통짜로 보일 수 있음.
- 연장 vs 새 예약 붙음은 피드만으로 구분 불가 → 잠정 경계는 사용자가 확인 카드에서 확정하기 전까지 "추정"임 (⚠가 그 표시).
